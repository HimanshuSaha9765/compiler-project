# CompilerX - Error Detector
# Phase 7 - Advanced with type checking

def detect_errors(tokens, symbol_table):
    """
    Detect duplicate declarations, undeclared variables, and simple type mismatches
    """
    symbols = symbol_table.get('symbols', [])
    
    # Build symbol lookup: name -> {type, scope_level}
    sym_map = {}
    for s in symbols:
        key = (s['name'], s['scope_level'])
        sym_map[key] = s
    
    # name -> first declaration (any scope, for undeclared check)
    declared_names = {}
    for s in symbols:
    ...
    # Undeclared variable + simple type checking
    undeclared_errors = []
    type_warnings = []
    error_id = 1
    
    # Scan for assignments: IDENTIFIER = <expr>
    i = 0
    while i < len(tokens)-2:
        t = tokens[i]
        if t['token_type'] == 'IDENTIFIER':
            # is this a variable use (not a declaration)?
            ...
            if not is_declaration:
                ...
                if name not in declared_names:
                    ...
                    # Avoid duplicate undeclared reports
    ...
    # Simple type mismatch detection: int/float var = string literal
    # Walk: TYPE IDENTIFIER = STRING_LITERAL
    i = 0
    while i < len(tokens)-3:
        if (tokens[i]['token_type'] == 'KEYWORD' and 
            ...
            if assigned_type == 'string' and var_type in ('int','float','double'):
                # Type mismatch warning – add to warnings list
                warnings.append({
                    'warning_id': warning_id,
